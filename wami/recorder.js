(function(window) {
	...
	Wami.setup = function(options) {
		...
		function setOptions(options) {
			...
			if (options.onShowSecurity) {
				_options.onShowSecurity = options.onShowSecurity;
			}
			...
			var containerNode = document.getElementById(_options.id);
			if (containerNode) {
				containerNode.appendChild(container);
			} else {
				_options.onError("Missing dom id");
			}
		...
		function checkSecurity() {
			...
			if (settings.microphone.granted) {
				...
				if (_options.onShowSecurity) {
					_options.onShowSecurity();
				}
